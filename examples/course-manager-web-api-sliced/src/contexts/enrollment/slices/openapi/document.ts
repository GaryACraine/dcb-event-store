...
export function buildOpenApiDocument(): object {
    ...
    return generator.generateDocument({
        openapi: "3.1.0",
        info: {
            title: "Course Manager API (Vertical Slice Architecture)",
            version: "1.0.0",
            description:
                "DCB Event Store example — vertical slice architecture with commands over HTTP, ETags, idempotency keys, read-model queries, and an SSE event feed."
        },
        servers: [{ url: "/" }]
    })
}
