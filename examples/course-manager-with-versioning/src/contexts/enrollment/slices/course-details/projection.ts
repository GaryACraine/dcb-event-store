        ...
        for (const sequencedEvent of events) {
            ...
            switch (sequencedEvent.event.type) {
                case "courseWasRegistered": {
                    /**
                     * Schema-version-aware projection.
                     *
                     * V1: { courseId, title, capacity } — no department or description
                     * V2: { courseId, title, capacity, department } — no description
                     * V3: { courseId, name, description, capacity, department } — current
                     *
                     * The read model normalises to a single shape. title comes from
                     * data.title (V1/V2) or data.name (V3); department defaults to
                     * "Unknown" for V1; description defaults to "" for V1/V2.
                     */
