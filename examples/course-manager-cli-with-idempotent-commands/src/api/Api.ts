...
export class Api {
    ...
    async registerCourse(cmd: { id: string; title: string; capacity: number; idempotencyKey?: string }) {
    ...
    async registerStudent(cmd: { id: string; name: string; idempotencyKey?: string }) {
        const { id, name } = cmd
        const { state, appendCondition } = await buildDecisionModel(this.eventStore, {
    ...
    async updateCourseCapacity(cmd: { courseId: string; newCapacity: number; idempotencyKey?: string }) {
    ...
    async updateCourseTitle(cmd: { courseId: string; newTitle: string; idempotencyKey?: string }) {
    ...
    async subscribeStudentToCourse(cmd: {
        courseId: string
        studentId: string
    ...
    async unsubscribeStudentFromCourse(cmd: { courseId: string; studentId: string; idempotencyKey?: string }) {
        const { courseId, studentId } = cmd
