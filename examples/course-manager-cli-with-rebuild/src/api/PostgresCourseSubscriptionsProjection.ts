...
export interface CourseDocV2 extends CourseDoc {
...
/**
 * V2 projection: same as v1 but adds a `subscriberCount` field to the course
 * document, materialised from subscription/unsubscription events.
 */
export const courseSubscriptionsProjectionV2 = pongoProjection({
    ...
    handle: async (events: SequencedEvent[], context: PongoProjectionContext) => {
        const courses = context.pongo.db().collection<CourseDocV2>("courses")
        const students = context.pongo.db().collection<StudentDoc>("students")
            ...
            switch (sequencedEvent.event.type) {
                ...
                case "studentWasSubscribed": {
                    ...
                    if (course) {
                        await courses.updateOne(
                            { _id: courseId },
                            { $set: { subscriberCount: (course.subscriberCount ?? 0) + 1 } }
                        )
                    }
                        ...
                        await courses.updateOne(
                            { _id: courseId },
                            {
                                $set: {
                                    ...
                                    subscriberCount: Math.max(0, (course.subscriberCount ?? 0) - 1)
                                }
                            }
                        )
                    }
    ...
    truncate: async pongo => {
        const db = pongo.db()
        await db.collection<CourseDocV2>("courses").deleteMany()
        await db.collection<StudentDoc>("students").deleteMany()
    }
})
